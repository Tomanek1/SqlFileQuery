...
namespace SqlFileQuery.Lib
{
	...
	public class SqlFileQuery : IDisposable
	{
		...
		protected string ParentAssembly { get; set; }
		...
		public SqlFileQuery(SqlConnection sqlConnection, string scriptsPath)
		{
			...
			this.ParentAssembly = this.GetParentAssembly();
		}
		...
		{
			string name = ParentAssembly + "." + ScriptsPath + "." + fileName;
			var assembly = Assembly.Load(ParentAssembly);
		...
		private string GetParentAssembly()
		{
			...
			object[] attributes = initialAssembly.GetCustomAttributes(typeof(AssemblyProductAttribute), false);
			string name = null;
			if (attributes.Length > 0)
			{
				name = (attributes[0] as AssemblyProductAttribute).Product;
			}
			return name;
		}
